#### KAUSHIK DUTTA ####
#### EfficientNet for classification - Training from Scratch for IHC Only #####
#### This code block implements an EfficientNet and a Confidence Loss Function for the prediction.
...
class EfficientNetClassifierWithConfNet(pl.LightningModule):
    def __init__(self, num_classes=4, learning_rate=0.0001, lambda_confidence=0.5, alpha=None, gamma=2.0, beta=0.5):
        super(EfficientNetClassifierWithConfNet, self).__init__()

        # Initialize EfficientNet model
        self.efficientnet = timm.create_model(
            'efficientnet_b0',  # You can change this to a different EfficientNet variant if needed
            pretrained=False,
            num_classes=0,
            in_chans=3
        )
        self.efficientnet.reset_classifier(0)      # Removes the classifier (sets output features to 0)
        ...
        self.classifier = ClassifierHead(in_features=self.efficientnet.num_features, num_classes=num_classes)
        self.confidence_net = ConfNet(num_classes=self.efficientnet.num_features)
...
tb_log_dir = base_log_dir / "tb_logs" / "EfficientNet_Scratch"
checkpoint_dir = base_log_dir / "saved_weights" / "EfficientNet_Scratch"
...
checkpoint_callback = ModelCheckpoint(
    ...
    filename="efficientnet_confnet-{epoch:02d}-{val_acc:.2f}"
)
...
max_epochs = 100
vit_confnet_model = EfficientNetClassifierWithConfNet(num_classes=4, learning_rate=0.0001, lambda_confidence = 0.75, alpha = class_weights_tensor, gamma = 2, beta = 0.3)
