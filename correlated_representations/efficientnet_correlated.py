...
##################################################################################### DATASET ###################################################################################################
with open('/ceph/chpc/home-nfs/d.kaushik/Vision_Transformer/Classification/correlated_representations/z_recon/attention/hz_train_'+ str(args.hidden_layer) +'.pkl', 'rb') as f:
...
########################################################### MODEL ARCHITECTURE ########################################
### EfficientNet Classifier (Full Model Fine-tuning)
class EfficientNetClassifier(pl.LightningModule):
    def __init__(self, num_classes=4, learning_rate=0.0001):
        super(EfficientNetClassifier, self).__init__()
        self.model = timm.create_model('efficientnet_b7', pretrained=False, num_classes = num_classes, in_chans = 6)
...
checkpoint_callback = ModelCheckpoint(
    ...
    monitor="val_acc",
    mode="max",
    dirpath="/ceph/chpc/home-nfs/d.kaushik/Vision_Transformer/Classification/correlated_representations/saved_weights/efficientnet/new/",
    filename="efficientnet_finetune_all_layer-{epoch:02d}-{val_loss:.2f}-{val_acc:.2f}"+"__"+str(args.hidden_layer)
)        
...
efficientnet_model = EfficientNetClassifier(num_classes=4, learning_rate=0.0001)
trainer.fit(efficientnet_model, train_dataloaders=train_dataloader, val_dataloaders=validation_dataloader)